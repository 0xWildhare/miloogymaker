...
function ViewBackground({ readContracts, blockExplorer, mainnetProvider, targetNetwork, totalSupply }) {
  let { id } = useParams();
  const fetchedId = useRef();

  //const rawTokenURI = useContractReader(readContracts, "Backgrounds", "tokenURI", [id]);
  const [tokenURI, setTokenURI] = useState();
  useEffect(() => {
    const getTokenData = async () => {
        try{
            let rawTokenURI = await readContracts.Background.tokenURI(id);
        ...
        <p>
          <a
            href={`https://${targetNetwork.name == "rinkeby" ? `testnets.` : ""}opensea.io/assets/${
              readContracts.Background.address
            }/${id}`}
            target="_blank"
          >
            OpenSea
          ...
          <span>{` / `}</span>
          <a href={`${blockExplorer}/token/${readContracts.Background.address}?a=${id}`} target="_blank">
            Etherscan
          </a>
        </p>
