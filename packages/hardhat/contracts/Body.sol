...
contract Body is ERC721Enumerable, Ownable {
  ...
  INftMetadata public bdContract;
  ERC721Enumerable public miloogy;

  mapping (uint256 => bytes32) public genes;

  constructor(address owner_, ERC721Enumerable miloogy_) ERC721("Miloogy Body", "MILBD") {
  ...
  function mintItem() public payable returns (uint256) {
      ...
      require(address(bdContract) != address(0), "NOT MINTING YET");
  ...
  function setBd(INftMetadata newBd) public onlyOwner {
    require(address(bdContract) == address(0), "can only set once");
    bdContract = newBd;
  }
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
    ...
    assert(address(bdContract) != address(0));
    return(bdContract.tokenURI(id));
  ...
  function renderTokenById(uint256 id) public view returns (string memory) {
      ...
      assert(address(bdContract) != address(0));
      return(bdContract.renderTokenById(id));
  ...
  function renderTokenByIdFront(uint256 id) public view returns (string memory) {
      ...
      assert(address(bdContract) != address(0));
      return(bdContract.renderTokenByIdFront(id));
  ...
  function renderTokenByIdBack(uint256 id) public view returns (string memory) {
      ...
      assert(address(bdContract) != address(0));
      return(bdContract.renderTokenByIdBack(id));
  ...
  function getTraits(uint id) public view returns(string memory) {
      ...
      assert(address(bdContract) != address(0));
      return(bdContract.getTraits(id));
